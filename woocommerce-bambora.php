...
class WC_Bambora extends WC_Payment_Gateway_CC {
	...
	function __construct() {
		...
		$this->method_title = __( "Bambora APAC ", 'wc-bambora' );

		// The description for this Payment Gateway, shown on the actual Payment options page on the backend
		$this->method_description = __( "Welcome to the Bambora APAC Plug-in for WooCommerce. Need an Account? Check us out at <a href='https:///www.bambora.com'>bambora.com</a>", 'wc-bambora' );

		// The title to be used for the vertical tabs that can be ordered top to bottom
		$this->title = __( "Bambora APAC ", 'wc-bambora' );
		...
		if ( $this->enabled_sandbox ) {
			$this->description .= ' ' . sprintf( __( 'Bambora Sandbox mode has been activated, you will be able to the use the card details mentioned on the following URL for testing.  "<a href="%s" target="_Blank" >Click here</a>".', 'woocommerce-bambora-gateway' ), 'http://dev.bambora.com/native-payment/guides/introduction/test-cards' );
	...
	public function load_admin_scripts() {
		...
		wp_enqueue_script( 'woocommerce_bambora_admin', plugins_url( 'assets/js/admin-bambora.js', WC_BAMBORA_MAIN_FILE ) );
	...
	public function init_form_fields() {		
		...
		$this->form_fields = array(
			'enabled' => array(
				'title'		=> __( 'Enable checkout', 'wc-bambora' ),
				'label'		=> __( 'Enable Bambora payment gateway', 'wc-bambora' ),
				'type'		=> 'checkbox',
				'default'	=> 'no',
			),
			'bambora_product' => array(
				'title'       => __( 'Bambora Product', 'wc-bambora' ),
				'type'        => 'select',
				'desc_tip'    => __( 'Get started as a small business with Bambora Ready or have access to our enterprise solution Bambora Advance', 'wc-bambora' ),
				'options'     => array(
					'ready' => __( 'Ready', 'wc-bambora' ),
					'advance' => __( 'Advance', 'wc-bambora' ),
				),
			),			
			'title' => array(
				'title'		=> __( 'Title', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'Payment title the customer will see during the checkout process', 'wc-bambora' ),
				'default'	=> __( 'Pay with Bambora', 'wc-bambora' ),
			),			
			'enabled_sandbox' => array(
				'title'		=> __( 'Enable sandbox mode', 'wc-bambora' ),
				'label'		=> __( 'Enable sandbox mode to test the payments', 'wc-bambora' ),
				'type'		=> 'checkbox',
				'default'	=> 'no',
			),
			'api_login' => array(
				'title'		=> __( 'Live API Username', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'The Live API username in your Bambora account', 'wc-bambora' ),
			),
			'api_password' => array(
				'title'		=> __( 'Live API Password', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'The Live API password in your Bambora account', 'wc-bambora' ),
			),
			'api_account' => array(
				'title'		=> __( 'Live API Account', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'The Live API account in your Bambora account', 'wc-bambora' ),
			),
			
			'test_api_login' => array(
				'title'		=> __( 'Test API Username', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'The Test API username in your Bambora account', 'wc-bambora' ),
			),
			'test_api_password' => array(
				'title'		=> __( 'Test API Password', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'The Test API password in your Bambora account', 'wc-bambora' ),
			),
			'test_api_account' => array(
				'title'		=> __( 'Test API Account', 'wc-bambora' ),
				'type'		=> 'text',
				'desc_tip'	=> __( 'The Test API account in your Bambora account', 'wc-bambora' ),
			),			
			'checkout_mode' => array(
				'title'       => __( 'Checkout Method', 'wc-bambora' ),
				'type'        => 'select',
				'desc_tip'    => __( 'Allow the customer to use the Bambora Integrated Checkout or the Built in payment page.', 'wc-bambora' ),
				'options'     => array(
					'api' => __( 'API', 'wc-bambora' ),
					//'integrated' => __( 'Integrated', 'wc-bambora' ),
				),
			),
			'dl' => array(
				'title'       => __( 'DL', 'wc-bambora' ),
				'type'        => 'text',
				'desc_tip'    => __( 'The styling options of the Bambora integrated checkout page', 'wc-bambora' ),
				'default'	=> 'checkoutv1_hpp_purchase',				
			),
			'save_card_detail' => array(
				'title'		=> __( 'Save card at checkout', 'wc-bambora' ),
				'label'		=> __( 'Enable merchant to save card details against account', 'wc-bambora' ),
				'type'		=> 'checkbox',
				'default'	=> 'no',
			),
			'save_card_method' => array(
				'title'       => __( 'Card Storage Method', 'wc-bambora' ),
				'type'        => 'select',
				'desc_tip'    => __( 'Select Card Storage Method', 'wc-bambora' ),
				'options'     => array(
					'tokenisation' => __( 'Tokenisation', 'wc-bambora' ),
					'customerregistration' => __( 'Customer Registration', 'wc-bambora' ),
				),
			),
			// 'save_cust_storage_no' => array(
			// 	'title'		=> __( 'Store Customer Storage Number?', 'wc-bambora' ),
			// 	'label'		=> __( 'Enable saving of customer storage number', 'wc-bambora' ),
			// 	'type'		=> 'checkbox',
			// 	'default'	=> 'no',
			// ),
			'payment_scheduling' => array(
				'title'		=> __( 'Enable Payment Scheduling', 'wc-bambora' ),
				'label'		=> __( 'Enable payment scheduling on your online store', 'wc-bambora' ),
				'type'		=> 'checkbox',
				'default'	=> 'no',
			),
			'batch_payment' => array(
				'title'		=> __( 'Enable Batch Payments', 'wc-bambora' ),
				'label'		=> __( 'Enable batch payments on your online store', 'wc-bambora' ),
				'type'		=> 'checkbox',
				'default'	=> 'no',
			),
			// 'batch_payment_type' => array(
			// 	'title'       => __( 'Batch Payment Type', 'wc-bambora' ),
			// 	'type'        => 'select',
			// 	'desc_tip'    => __( 'Select batch payment type', 'wc-bambora' ),
			// 	'options'     => array(
			// 		'tokenformat' => __( 'Token Format', 'wc-bambora' ),
			// 		'registeredcustomerformat' => __( 'Registered Customer Format', 'wc-bambora' ),
			// 	),
			// ),
			'red_3dsec' => array(
				'title'		=> __( '3D Secure or RED Enabled?', 'wc-bambora' ),
				'label'		=> __( 'Have you enabled 3D Secure or RED Enabled on your Bambora account? ', 'wc-bambora' ),
				'type'		=> 'checkbox',
				'default'	=> 'no',
			)
	...
	public function payment_fields() {
		...
		if ( is_add_payment_method_page() ) {
			$pay_button_text = __( 'Add Card', 'woocommerce-gateway-bambora' );
			$total        = '';
		} else {
			$pay_button_text = '';
		}
		...
		if($this->checkout_mode=='api'){
			...
			//wp_enqueue_script( 'woocommerce_bambora', plugins_url( 'assets/js/checkout-bambora.js', WC_BAMBORA_MAIN_FILE ) );
	...
	public function process_payment( $order_id ) {
		...
		include_once( dirname( __FILE__ ) . '/lib/woocommerce-bambora-api.php' );
		...
		if($this->checkout_mode=='api'){	
			...
			if($this->save_card_detail=='yes'){
				...
				if(isset($_POST['wc-wc_bambora-payment-token']) && trim($_POST['wc-wc_bambora-payment-token'])!='new'){
					$token_id = trim($_POST['wc-wc_bambora-payment-token']);
					...
					// If its a new card?
					if(isset($_POST['wc-wc_bambora-new-payment-method']) && $_POST['wc-wc_bambora-new-payment-method']==true){
			...
			$APIRequest = new WooCommerce_Bambora_Api($this->wsdl_url, array("trace" => 1)); 
	...
	public function admin_notices() {
		...
		if ( ( function_exists( 'wc_site_is_https' ) && ! wc_site_is_https() ) && ( 'no' === get_option( 'woocommerce_force_ssl_checkout' ) && ! class_exists( 'WordPressHTTPS' ) ) ) {
			echo '<div class="error bambora-ssl-message"><p>' . sprintf( __( 'Hey we are on test mode! We noticed that your checkout is not secured as force SSL option is not enabled. 
				<a href="%s">Click here</a>', 'woocommerce-gateway-bambora' ), admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) . '</p></div>';
		}
	}
	// End admin_notices
	...
	function bp_postypesmetabox() { 
		add_meta_box( 
			'bp-event-metabox', 
			__( 'Schedule Information', 'wc-bambora' ), 
	...
	function bp_metabox_callback( $post ) { 
		...
		?> 
		<p>	
		<select class="widefat" id="CustNumber" name="CustNumber" maxlength="150" required placeholder="<?php _e( 'User ID', 'wc-bambora' ); ?>" >
		...
		<p>	
		<label for="Amount"><?php _e( 'Amount', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="Amount" type="text" name="Amount" maxlength="150" required placeholder="<?php _e( 'Amount', 'wc-bambora' ); ?>" value="<?php echo  $Amount ; ?>" />
		</p>

		<p>
		<label for="StartDate"><?php _e( 'Start Date', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="StartDate" type="text" name="StartDate" required maxlength="10" placeholder="<?php _e( 'Use datepicker', 'wc-bambora' ); ?>" value="<?php echo date_i18n( $dateformat, esc_attr( $StartDate ) ); ?>" />
		</p>	

		<p>	
		<label for="Frequency"><?php _e( ' Frequency', 'wc-bambora' ); ?></label> 
		<select class="widefat" id="Frequency" name="Frequency" maxlength="150" required placeholder="<?php _e( 'Frequency', 'wc-bambora' ); ?>" >
			...
			<?php 
					...
					foreach ($this->frequency as $Frequency_list_key => $Frequency_list_value) {
						...
						echo "<option value='".$Frequency_list_key."' ".$sel.">".$Frequency_list_value."</option>";
					}
		...
		<label for="enableEndDate"><?php _e('Enable End Date for Schedule', 'wc-bambora'); ?></label>
		</p>

		<p>
		<label for="EndPaymentDate"><?php _e( 'End Payment Date', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="EndPaymentDate" type="text" name="EndPaymentDate"  maxlength="10" placeholder="<?php _e( 'Use datepicker', 'wc-bambora' ); ?>" value="<?php echo date_i18n( $dateformat, esc_attr( $EndPaymentDate ) ); ?>" />
		</p>
		...
		<label for="enableEndPaymentCount"><?php _e('Enable Payment Count for Schedule', 'wc-bambora'); ?></label>
		</p>

		<p>
		<label for="EndPaymentCount"><?php _e( 'End Payment Count', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="EndPaymentCount" type="text" name="EndPaymentCount" maxlength="10" placeholder="<?php _e( 'End Payment Count', 'wc-bambora' ); ?>" value="<?php echo  $EndPaymentCount ; ?>" />
		</p>

		<p>
		<label for="ScheduleID"><?php _e( 'Schedule ID', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="ScheduleID" type="text" name="ScheduleID" maxlength="15" placeholder="<?php _e( 'Schedule ID', 'wc-bambora' ); ?>" value="<?php echo  $ScheduleID ; ?>" />
		</p>
	...
	function bp_save_bspayment( $post_id ) { 
	    ...
	    if ( isset( $_POST['CustNumber'] ) ) { 
			...
			$params['CustNumber'] = $_POST['CustNumber'];
		}else{
			$error = '1';
		}
		...
		if ( isset( $_POST['Frequency'] ) ) { 
			...
			$params['Frequency'] = $_POST['Frequency'];
		}else{
			$error = '1';
		} 	
	    ...
	    if ( isset( $_POST['EndPaymentDate'] ) ) { 
			...
			if ( isset( $_POST['enableEndDate'] ) ) { 
				$params['EndPaymentDate'] = $_POST['EndPaymentDate'];
			}
		} 
		...
		if ( isset( $_POST['EndPaymentCount'] ) ) { 
			...
			if ( isset( $_POST['enableEndPaymentCount'] ) ) { 
				$params['EndPaymentCount'] = $_POST['EndPaymentCount'];
			}
		}

	    if ( isset( $_POST['ScheduleID'] ) ) { 
			update_post_meta( $post_id, 'ScheduleID', sanitize_text_field( $_POST['ScheduleID']  ) ); 
			$params['ScheduleID'] = $_POST['ScheduleID'];
		} 
		...
		if (  $transactionResponse->ResponseCode == '0'  ) {
			...
			// Get user			
			$usr = get_user_by('ID',$_POST['CustNumber']);
	...
	function bb_postypesmetabox() { 
		add_meta_box( 
			'bb-event-metabox', 
			__( 'Batch Payment Information', 'wc-bambora' ), 
	...
	function bb_metabox_callback( $post ) { 
		...
		<p>	
		<label for="BatchID"><?php _e( 'Batch ID', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="BatchID" type="text" name="BatchID" maxlength="150" required placeholder="<?php _e( 'Batch ID', 'wc-bambora' ); ?>" value="<?php echo  $BatchID ; ?>" />
		</p>
		...
		<tr>
			...
			<td>
			<select class="widefat" id="CustNumber" name="CustNumber_<?php echo $i; ?>" maxlength="150"  placeholder="<?php _e( 'User ID', 'wc-bambora' ); ?>" >
				<option value="0">Select User</option>
				<?php 
					if(count($use_list)>0){
			...
			<td><input class="widefat" id="Amount" type="text" name="Amount_<?php echo $i; ?>" maxlength="150"  placeholder="<?php _e( 'Amount', 'wc-bambora' ); ?>" value="<?php echo  get_post_meta( $post->ID, 'Amount_'.$i, true ) ; ?>" /></td>			
		</tr>
		...
		<p>
		<label for="Totaltransactions"><?php _e( 'Total Transactions', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="Totaltransactions" type="text" name="Totaltransactions" maxlength="10" placeholder="<?php _e( 'Total Transactions', 'wc-bambora' ); ?>" value="<?php echo  $Totaltransactions ; ?>" />
		</p>

		<p>
		<label for="Totalamount"><?php _e( 'Total Amount', 'wc-bambora' ); ?></label> 
		<input class="widefat" id="Totalamount" type="text" name="Totalamount" maxlength="10" placeholder="<?php _e( 'Total Amount', 'wc-bambora' ); ?>" value="<?php echo  $Totalamount ; ?>" />
		</p>
	...
	function bb_save_bspayment( $post_id ) { 
	   ...
	   for ($i=0; $i < $this->maxbatchrows; $i++) { 
	   	...
	   	$wpdb->update( 
				'wp_posts', 
				array( 
					'post_title'   => $_POST['BatchID'].'-'.date('Y').date('m').date('d').date('h').date('i').date('s')			
				), 
				array( 'ID' => $post_id )
	...
	function perform_api_upload($post_id) {
		...
		$APIRequest = new WooCommerce_Bambora_Api($this->wsdl_batch_url, array("trace" => 1)); 
