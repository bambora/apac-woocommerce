...
class Bambora_Apac extends WC_Payment_Gateway_CC
{
    ...
    function __construct()
    {
        ...
        if ($this->payment_scheduling == 'yes' && $this->checkout_mode == 'api') {
        ...
        if ($this->batch_payment == 'yes' && $this->checkout_mode == 'api') {
    ...
    public function process_payment($order_id)
    {
        ...
        include_once(plugin_dir_path(__FILE__) . 'lib/bambora-apac-api.php');
        ...
        $isApi = false;
        if ($this->checkout_mode != 'api' && $_POST['wc-bambora_apac-payment-token'] != 'new') {
            $isApi = true;
        }
        if ($this->checkout_mode == 'api' || $isApi) {
...
/**************************************************************/
            ...
            $SST = $params['SST'];
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $this->IntegratedUrl . "?UserName=$UserName&Password=$Password&SST=$SST&SessionId=$sessionid&Query=true");
            ...
            if (curl_exec($ch) === false) {
                ...
                $arr = explode('<input type="hidden"', $server_output); //print_r($arr); die;
                if (count($arr) > 0) {
                    $creditCard = explode('******', $this->getExplodeValues($arr[6], 2));
                    $expiryDate =explode('/',$this->getExplodeValues($arr[8], 2)) ;
                    $userId = $this->getExplodeValues($arr[5], 2);
                    $month=$expiryDate[0];
                    $year='20'.$expiryDate[1];

                    $token = new WC_Payment_Token_CC();
                    $token->set_token((string)'9160688351712442');
                    $token->set_card_type(strtolower('Credit card'));
                    $token->set_gateway_id('bambora_apac');
                    $token->set_last4($creditCard[1]);
                    $token->set_expiry_month((string)$month);
                    $token->set_expiry_year((string)$year);
            ...
            echo '<img src="' . WP_PLUGIN_URL . '/' . plugin_basename(dirname(__FILE__)) . '/assets/images/success.png' . '" style=" margin: 40px auto 20px; display: block;">';
            echo '<p style="font-size: 30px; text-align: center; margin: 50px 0 10px; font-family: "Fakt Pro", "HelveticaNeue-Light", "Helvetica Neue Light", "Helvetica Neue", Helvetica, Arial, "Lucida Grande", sans-serif;">Your payment was successful!</p>';
            echo '<a style="text-align: center;display: block;color: #664c8e;    text-decoration: none;font-size: 15px;font-family: "Fakt Pro", "HelveticaNeue-Light", "Helvetica Neue Light", "Helvetica Neue", Helvetica, Arial, "Lucida Grande", sans-serif;" href="' . site_url() . '/checkout/order-received/' . $order_id . '/?key=' . $order_key[0] . '" target="_parent">click here to view confirmation</a>';
    ...
    public function getExplodeValues($att, $key)
    {
        $str = trim(str_replace(array('/>', '"'), '', $att));
        $arr_inner = explode('=', $str);

        return $arr_inner[$key];
    }
    ...
    public function receipt_page($order)
    {
        ...
        include_once(plugin_dir_path(__FILE__) . 'lib/bambora-apac-integrate.php');
        global $woocommerce;
        ...
        $return_arr = $Int_Request->getSST($this->IntegratedUrl, $url_arr);
            ...
            echo '<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <iframe style="border:none;width:100%;height:415px;" id="payment-iframe"  name="payment-iframe"></iframe>
    ...
    public function add_payment_method()
    {
        ...
        if ($this->checkout_mode == 'api') {
            ...
            if ($cr == '1') {
                $op = $params['operation'] = 'RegisterSingleCustomer';
                $params['CustNumber'] = get_current_user_id();
            }
            ...
            if ($transactionResponse->ReturnValue == '0') {
                ...
                if ($cr && get_current_user_id() && class_exists('WC_Payment_Token_CC')) {
    ...
    function bp_metabox_callback($post)
    {
        ...
        ?>
        <p>
            <select class="widefat" id="CustNumber" name="CustNumber" maxlength="150" required
                    placeholder="<?php _e('User ID', 'bambora-apac'); ?>">
        ...
        <p>
            ...
            <input class="widefat" id="Amount" type="text" name="Amount" maxlength="150" required
                   placeholder="<?php _e('Amount', 'bambora-apac'); ?>" value="<?php echo esc_html($Amount); ?>"/>
        </p>

        <p>
            <label for="StartDate"><?php _e('Start Date', 'bambora-apac'); ?></label>
            <input class="widefat" id="StartDate" type="text" name="StartDate" required maxlength="10"
                   placeholder="<?php _e('Use datepicker', 'bambora-apac'); ?>"
                   value="<?php echo date_i18n($dateformat, esc_attr($StartDate)); ?>"/>
        </p>

        <p>
            <label for="Frequency"><?php _e(' Frequency', 'bambora-apac'); ?></label>
            <select class="widefat" id="Frequency" name="Frequency" maxlength="150" required
                    placeholder="<?php _e('Frequency', 'bambora-apac'); ?>">
        ...
        <p>
            <input class="checkbox" id="Enable End Date" type="checkbox" name="enableEndDate"
                   value="yes" <?php checked($enableEndDate, 'yes'); ?> />
        ...
        <p>
            ...
            <input class="widefat" id="EndPaymentDate" type="text" name="EndPaymentDate" maxlength="10"
                   placeholder="<?php _e('Use datepicker', 'bambora-apac'); ?>"
                   value="<?php echo date_i18n($dateformat, esc_attr($EndPaymentDate)); ?>"/>
        </p>

        <p>
            <input class="checkbox" id="enableEndPaymentCount" type="checkbox" name="enableEndPaymentCount"
                   value="yes" <?php checked($EndPaymentCount, 'yes'); ?> />
        ...
        <p>
            ...
            <input class="widefat" id="EndPaymentCount" type="text" name="EndPaymentCount" maxlength="10"
                   placeholder="<?php _e('End Payment Count', 'bambora-apac'); ?>"
                   value="<?php echo esc_html($EndPaymentCount); ?>"/>
        </p>

        <p>
            <label for="ScheduleID"><?php _e('Schedule ID', 'bambora-apac'); ?></label>
            <input class="widefat" id="ScheduleID" type="text" name="ScheduleID" maxlength="15"
                   placeholder="<?php _e('Schedule ID', 'bambora-apac'); ?>"
                   value="<?php echo esc_html($ScheduleID); ?>"/>
        </p>
    ...
    function bp_save_bspayment($post_id)
    {
        ...
        if (isset($_POST['Amount'])) {
            ...
            $params['Amount'] = sanitize_text_field($_POST['Amount']) * 100;
        } else {
            $error = '1';
        }
    ...
    function bb_metabox_callback($post)
    {
        ...
        <p>
            ...
            <input class="widefat" id="BatchID" type="text" name="BatchID" maxlength="150" required
                   placeholder="<?php _e('Batch ID', 'bambora-apac'); ?>" value="<?php echo esc_html($BatchID); ?>"/>
        </p>
        ...
        <table style="width:50%;">
            ...
            for ($i = 0; $i < $this->maxbatchrows; $i++) {
                ...
                <tr>
                    ...
                    <td>
                        <select class="widefat" id="CustNumber" name="CustNumber_<?php echo $i; ?>" maxlength="150"
                                placeholder="<?php _e('User ID', 'bambora-apac'); ?>">
                            <option value="0">Select User</option>
                            <?php
                            if (count($use_list) > 0) {
                    ...
                    <td><input class="widefat" id="Amount" type="text" name="Amount_<?php echo $i; ?>" maxlength="150"
                               placeholder="<?php _e('Amount', 'bambora-apac'); ?>"
                               value="<?php echo get_post_meta($post->ID, 'Amount_' . $i, true); ?>"/></td>
                </tr>
        ...
        <p>
            ...
            <input class="widefat" id="Totaltransactions" type="text" name="Totaltransactions" maxlength="10"
                   placeholder="<?php _e('Total Transactions', 'bambora-apac'); ?>"
                   value="<?php echo esc_html($Totaltransactions); ?>"/>
        </p>

        <p>
            <label for="Totalamount"><?php _e('Total Amount', 'bambora-apac'); ?></label>
            <input class="widefat" id="Totalamount" type="text" name="Totalamount" maxlength="10"
                   placeholder="<?php _e('Total Amount', 'bambora-apac'); ?>"
                   value="<?php echo esc_html($Totalamount); ?>"/>
        </p>
    ...
    function custom_bulk_action()
    {
        ...
        if ($post_type == 'bbpayment_postype') {
            ...
            wp_enqueue_script('bambora_footer', plugins_url('assets/js/bulkactions.js', BAMBORA_APAC_MAIN_FILE));
        ...
        if ($this->save_card_method == "customerregistration") {
            ...
            for ($i = 0; $i < $this->maxbatchrows; $i++) {
                if (isset($post_meta['CustNumber_' . $i][0]) && isset($post_meta['Amount_' . $i][0])) {
                    ...
                    echo $this->Account . ",1," . $cust[1] . "," . $cust[1] . "," . $amount . ",,,,,,\n\n";
                }
            }
    ...
    function perform_api_upload($post_id)
    {
        ...
        if ($this->save_card_method == "customerregistration") {
            ...
            for ($i = 0; $i < $this->maxbatchrows; $i++) {
                if (isset($post_meta['CustNumber_' . $i][0]) && isset($post_meta['Amount_' . $i][0])) {
                    $cust = explode(':', $post_meta['CustNumber_' . $i][0]);
                    $amount = $post_meta['Amount_' . $i][0] * 100;
                    $var .= $this->Account . ",1," . $cust[1] . "," . $cust[1] . "," . $amount . ",,,,,,\n\n";
                }
            }
        ...
        if ($this->save_card_method == "customerregistration") {
            $xmlRequest = $APIRequest->createSubmitBatchv2TrnFileBody($params, $this->action_batch_url);
        } else {
        ...
        if ($transactionResponse['value'] == '0') {
            ...
            if ($this->save_card_method == "customerregistration") {
                $xmlRequest = $APIRequest->createAuthoriseBatchv2TrnFileBody($params, $this->action_batch_url);
            } else {
